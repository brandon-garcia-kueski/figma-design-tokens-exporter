...
const defaultValueTransformer = (propertyGroupValues) => {
  ...
  Object.keys(propertyGroupValues).forEach(function (key) {
    ...
    if (
      Object.prototype.hasOwnProperty.call(propertyGroupValues[key], "value")
    ) {
      transformedProperties[key] = originalFormat(propertyGroupValues[key]);
    }
    // if there is more nesting
    else {
      transformedProperties[key] = defaultValueTransformer(
        propertyGroupValues[key]
      );
    }
  });
...
const effectValueTransformer = (extractedValues) => {
  const values = extractedValues.map((effect) =>
    ["dropShadow", "innerShadow"].includes(effect.effectType.value)
      ? effectShadowValueTransformer(effect)
      : effectBlurValueTransformer(effect)
  );
  // turn array with only one item into normal object
...
const valueTransformer: {} | undefined = {
  ...
  icon: defaultValueTransformer,
};

const transformer = (
  token: internalTokenInterface
): OriginalFormatTokenInterface => {
  return {
    name: token.name,
    category: token.category,
    exportKey: token.extensions[config.key.extensionPluginData].exportKey,
    ...(token.description &&
      token.description.length > 0 && { comment: token.description }),
