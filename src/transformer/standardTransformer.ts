...
const lineHeightToDimension = (values): number => {
  ...
  if (values.lineHeight.unit === "percent") {
    return roundWithDecimals(
      values.fontSize.value * (values.lineHeight.value / 100),
      3
    );
  }
...
const letterSpacingToDimensions = (values): number => {
  ...
  if (values.letterSpacing.unit === "percent") {
    return roundWithDecimals(
      values.fontSize.value * (values.letterSpacing.value / 100),
      3
    );
  }
  return 0;
...
const widthToDimensionTransformer = ({
  values,
}): StandardTokenDataInterface => ({
...
const gridValueTransformer = (
  { values } /*: {values: extractedGridValues[]} */
): StandardTokenDataInterface | StandardTokenGroup => {
...
const gradientValueTransformer = ({
  gradientType,
  rotation,
  stops,
  opacity,
...
const fillValueTransformer = (
  ...
  if (
    ...
    token.extensions[config.key.extensionPluginData].alias
  ) {
    return {
      type: Object.hasOwnProperty.call(token.values[0], "fill")
        ? "color"
        : "custom-gradient",
...
const iconValueTransformer = ({ values }): StandardTokenDataInterface => ({
  type: "custom-icon" as StandardTokenTypes,
  value: {
    path: values.path,
  },
});
...
const valueTransformer = {
  ...
  icon: iconValueTransformer,
};

const transformTokens = (
  token: internalTokenInterface
): StandardTokenDataInterface | StandardTokenGroup => {
  return valueTransformer[token.category](token);
};

const transformer = (
  token: internalTokenInterface
): StandardTokenInterface | StandardTokenGroup => {
