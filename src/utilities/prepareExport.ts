...
const createTypographyTokens = (tokens: internalTokenInterface[], format) => {
  if (format === "standard") {
    return JSON.parse(
      JSON.stringify(
        tokens.filter((item) => item.category === tokenTypes.font.key)
      )
    ).map((item) => {
      item.name =
        "typography/" +
        item.name
          .substr(item.name.indexOf("/") + 1)
          .trim()
          .trimStart();
      ...
      item.extensions[config.key.extensionPluginData].exportKey = tokenTypes
        .typography.key as tokenCategoryType;
      return item;
    });
  }
  return [];
...
export const prepareExport = (tokens: string, settings: Settings) => {
  ...
  tokenArray = [
    ...tokenArray,
    ...createTypographyTokens(tokenArray, settings.tokenFormat),
  ];
  // filter by user setting for export keys
  const tokensFiltered: internalTokenInterface[] = tokenArray.filter(
    ({ exportKey }) => settings.exports[exportKey]
  );
  // add to name
  const prefixedTokens = prefixTokenName(tokensFiltered, settings);

  // converted values
  const tokensConverted = prefixedTokens.map((token) =>
    tokenTransformer[settings.tokenFormat](token)
  );
