...
import extractIcons from "../extractor/extractIcons";

const getPrefixArray = (prefixString: string = "") =>
  prefixString.split(",").map((item) => item.replace(/\s+/g, ""));
...
export const exportRawTokenArray = (figma: PluginAPI, settings: Settings) => {
  ...
  return [
    ...extractSizes(
      figmaData.tokenFrames,
      getPrefixArray(settings.prefix.size)
    ),
    ...extractBreakpoints(
      figmaData.tokenFrames,
      getPrefixArray(settings.prefix.breakpoint)
    ),
    ...extractSpacing(
      figmaData.tokenFrames,
      getPrefixArray(settings.prefix.spacing)
    ),
    ...extractBorders(
      figmaData.tokenFrames,
      getPrefixArray(settings.prefix.border)
    ),
    ...extractRadii(
      figmaData.tokenFrames,
      getPrefixArray(settings.prefix.radius)
    ),
    ...extractMotion(
      figmaData.tokenFrames,
      getPrefixArray(settings.prefix.motion)
    ),
    ...extractOpacities(
      figmaData.tokenFrames,
      getPrefixArray(settings.prefix.opacity)
    ),
    ...extractColors(figmaData.paintStyles, {
      color: getPrefixArray(settings.prefix.color),
      gradient: getPrefixArray(settings.prefix.gradient),
      alias: getPrefixArray(settings.alias),
    }),
    ...
    ...extractEffects(
      figmaData.effectStyles,
      getPrefixArray(settings.prefix.effect)
    ),
    ...extractIcons(
      figmaData.tokenFrames,
      getPrefixArray(settings.prefix.icon)
    ),
  ];
};
