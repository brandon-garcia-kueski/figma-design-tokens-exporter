...
const isTokenFrame = (node): boolean => {
  return (
    node.type === "FRAME" &&
    node.name.trim().toLowerCase().substr(0, tokenFrameName.length) ===
      tokenFrameName
  );
};

// return only nodes that are frames
const getFrameNodes = (nodes): FrameNode[] => [
  ...nodes
    .map((page) => page.findChildren((node) => isTokenFrame(node)))
    ...
    tokenFrames
      ...
      .map((item): customTokenNode[] => {
        if (item.type === "RECTANGLE" || item.type === "FRAME") {
          console.warn(
            "Please use only main components and variants, other types may be deprecated as tokens in the future"
          );
        }
          ...
          return [
            {
              name: item.name,
              path:
                item.vectorPaths.length > 0
                  ? item.vectorPaths[0].data
                  : undefined,
            },
          ];
        }
