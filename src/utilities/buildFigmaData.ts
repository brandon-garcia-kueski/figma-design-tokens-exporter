...
const buildFigmaData = (
  ...
  return {
    ...
    paintStyles: getPaintStyles(figma.getLocalPaintStyles()).filter((item) =>
      filterByPropertyName(item, userExclusionPrefixes)
    ),
    gridStyles: getGridStyles(figma.getLocalGridStyles()).filter((item) =>
      filterByPropertyName(item, userExclusionPrefixes)
    ),
    textStyles: getTextStyles(figma.getLocalTextStyles()).filter((item) =>
      filterByPropertyName(item, userExclusionPrefixes)
    ),
    effectStyles: getEffectStyles(figma.getLocalEffectStyles()).filter((item) =>
      filterByPropertyName(item, userExclusionPrefixes)
    ),
  };
};
